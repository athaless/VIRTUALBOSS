...
<% VerificaDireito "|DEL|", BuscaDireitosFromDB("modulo_CHAMADO", Request.Cookies("VBOSS")("ID_USUARIO")), true %>
...
<%
 ...
 Const auxAVISO  = "dlg_warning.gif:ATENÇÃO!Você está prestes a remover o registro <br> acima visualizado. Para confirmar clique no botão [ok], para desistir clique em [cancelar]."
   ...
   If strCODIGO <> "" Then
	  ...
	  strSQL = "SELECT * FROM CH_CHAMADO WHERE COD_CHAMADO = " & strCODIGO
...
<body>
<%=athBeginDialog(WMD_WIDTH, "Chamado - Dele&ccedil;&atilde;o")%>     
<% for Idx = 0 to objRS.fields.count - 11  'NÃO QUIZ EXIBIR TODOS OS DADOS... %>
...
<form name="form_delete" action="../_database/athDeleteToDB.asp" method="post">
       <input type="hidden" name="DEFAULT_TABLE"    value="CH_CHAMADO">
       <input type="hidden" name="DEFAULT_DB"       value="<%=CFG_DB%>">
       <input type="hidden" name="FIELD_PREFIX"     value="DBVAR_">
       <input type="hidden" name="RECORD_KEY_NAME"  value="COD_CHAMADO">
