					...
					<tr>
						...
						<td width="50">
							<select name="var_tipo" class="edtext" size="1">
								<%=MontaCombo("STR","SELECT TIPO, DESCRICAO FROM SYS_ENTIDADE WHERE TIPO='ENT_CLIENTE' ORDER BY ORDEM, DESCRICAO","TIPO","DESCRICAO","")%>
							</select>
						</td>
						<td width="5"></td>
...
<%
	strSQL = " SELECT COD_CLIENTE AS CODIGO, RAZAO_SOCIAL AS NOME, EMAIL, DT_INATIVO FROM ENT_CLIENTE " 
...
<table cellpadding="0" cellspacing="1" width="99%" align="center">
	...
	<tr bgcolor="#CCCCCC" bordercolordark="#CCCCCC" bordercolorlight="#CCCCCC" class="arial11Bold">  
		...
		<td width="91%"><div style="padding-left:3px; padding-right:3px;">Nome/Razão Social</div></td>
	</tr>
<%
      while not objRS.Eof
  	     strCor = "#DAEEFA"
