...
<% VerificaDireito "|PREVIEW|", BuscaDireitosFromDB("modulo_FIN_NF", Request.Cookies("VBOSS")("ID_USUARIO")), true %>
<%
...
<%	
Dim objConn, strSQL
Dim objRS1, objRS2, objRS3, objRS4
Dim objFSO, objText
Dim Cont1, Cont2, Cont3
Dim intCodNF, strMSG
Dim strDT_AGORA, strDT_VCTO, strUSUARIO, strDESCRICAO1, strDESCRICAO2
Dim strSAIDA1, strSAIDA2, strSAIDA3, strARQUIVO_LEITURA, strARQUIVO_ESCRITA1, strARQUIVO_ESCRITA2, strSEQUENCIA
...
If intCodNF <> "" Then
	...
	If Not objRS1.eof Then
		...
		' 5) Prepara valores
		'-----------------------------------
		IF (GetValue(objRS2, "ULT_NUM_FORM")<>"") THEN 
			strNUM_FORM = CDbl(GetValue(objRS2, "ULT_NUM_FORM")) + 1
		ELSE
			strNUM_FORM = 0
		END IF
		...
		'----------------------------------------------------------
		' 9) Grava arquivo da nota gerado a partir do modelo
		'----------------------------------------------------------
		If strMSG = "" Then
	...
	Else
		strSAIDA3 = Replace(strSAIDA3, "</body>", "<div id='Layer1' style='z-index:100; position:absolute; top:0px; left:0px; margin-top:100px; margin-left:100px;'><img src='../img/preview_nfrecibo.gif' /></div></body>")
