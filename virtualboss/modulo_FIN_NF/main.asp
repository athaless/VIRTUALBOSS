...
Response.Expires = -1
 VerificaDireito "|VIEW|", BuscaDireitosFromDB("modulo_FIN_NF", Request.Cookies("VBOSS")("ID_USUARIO")), true %>
 <!--#include file="../_database/athUtils.asp"-->
<%
  Dim objConn, objRS, strSQL 
  Dim strAviso, strSituacao, strNumNF, strSerieNF, strCodCfgNF
...
<table align="center" cellpadding="0" cellspacing="1" style="width:100%" class="tablesort">
 ...
 <thead>
   <tr> 
		...
		<th width="33%" class="sortable">Cliente</th>
        <th width="01%" class="sortable-currency" nowrap>ISSQN</th>
        <th width="01%" class="sortable-currency" nowrap>IRPJ</th>
        <th width="01%" class="sortable-currency" nowrap>COFINS</th>
        <th width="01%" class="sortable-currency" nowrap>PIS</th>
        <th width="01%" class="sortable-currency" nowrap>CSSS</th>
        <th width="01%" class="sortable-currency" nowrap>IRRF</th>
        <th width="01%" class="sortable-currency" nowrap>IRRF Ac</th>
        <th width="01%" class="sortable-currency" nowrap>Reducao Ac</th>
        <th width="01%" class="sortable-currency" nowrap>Total Imp. Cli</th>
		<th width="01%" class="sortable-currency" nowrap>Total Imp.</th>
		<th width="01%" class="sortable-currency" nowrap>Total Servi&ccedil;o</th>
		<th width="01%" class="sortable-currency" nowrap>Total NF</th>
		<th width="01%" class="sortable">Situa&ccedil;&atilde;o</th>
		<th width="01%" class="sortable-date-dmy" nowrap>Data Emiss&atilde;o</th>
		<th width="01%"></th>
	</tr>
  </thead>
...
<% 
	...
	%>
	<tr bgcolor=<%=strColor%>>
		<td><% If GetValue(objRS,"SITUACAO") <> "CANCELADA" Then Response.Write(MontaLinkGrade("modulo_FIN_NF","Delete.asp",GetValue(objRS,"COD_NF"),"IconAction_DEL.gif","REMOVER")) %></td>
		<td><% If GetValue(objRS,"SITUACAO") = "EM_EDICAO" Or GetValue(objRS,"SITUACAO") = "NAO_EMITIDA" Then Response.Write(MontaLinkGrade("modulo_FIN_NF","Update.asp",GetValue(objRS,"COD_NF"),"IconAction_EDIT.gif","ALTERAR")) %></td>
		<td><% Response.Write(MontaLinkGrade("modulo_FIN_NF","Detail.asp",GetValue(objRS,"COD_NF"),"IconAction_DETAIL.gif","DETALHES")) %></td>
		<td>
		  <% 
		   If GetValue(objRS,"SITUACAO") = "NAO_EMITIDA" Then 
			  Response.Write(MontaLinkPopup("modulo_FIN_NF","View.asp",GetValue(objRS,"COD_NF"),"Icon_BOLETO.gif","PREVIEW","840","550","no"))
           End IF			 
		  %>
		</td>
		<td>
		  <% 
		   If GetValue(objRS,"SITUACAO") = "NAO_EMITIDA" Then 
			  Response.Write(MontaLinkPopup("modulo_FIN_NF","Print.asp",GetValue(objRS,"COD_NF"),"IconAction_PRINTNF.gif","EMITIR","840","550","no"))
           End IF			 
		  %>
		</td>
		<td>
		  <%
		   If GetValue(objRS,"SITUACAO") = "EMITIDA" And GetValue(objRS,"TOTAL") = "0" Then 
		     Response.Write(MontaLinkPopup("modulo_FIN_NF","InsertTitulo.asp",GetValue(objRS,"COD_NF"),"IconAction_GERACP.gif","GERAR CONTA","600","570","no")) 
		   End if
		  %>	   
		</td>
		<td>
		  <% 
		   If GetValue(objRS,"SITUACAO") = "EMITIDA" And GetValue(objRS,"TOTAL") = "0" Then 
		     Response.Write(MontaLinkGrade("modulo_FIN_NF","AssociarTitulo.asp",GetValue(objRS,"COD_NF"),"IconAction_ASSOCIACP.gif","ASSOCIAR A UMA CONTA EXISTENTE")) 
		   End if
		  %>	   
		</td>
		<td nowrap><%=GetValue(objRS,"NUM_NF")%></td>
		...
		%></td>
		<td align="right" nowrap><%=PrepData(GetValue(objRS,"DT_EMISSAO"),true,false)%></td>
		<td>
		<% 
		If (GetValue(objRS,"ARQUIVO") <> "") And (GetValue(objRS,"SITUACAO") = "EMITIDA" Or GetValue(objRS,"SITUACAO") = "CANCELADA") Then 
			strFilePath = "upload/" & Request.Cookies("VBOSS")("CLINAME") & "/FIN_Notas"
			Response.Write(MontaLinkPopup(strFilePath,GetValue(objRS,"ARQUIVO"),"","Icon_BOLETO.gif","NOTA_FISCAL/RECIBO","840","550","yes")) 
		End If
		%>
		</td>
	</tr>
