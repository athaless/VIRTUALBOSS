...
If intCodNF <> "" Then
	...
	strSQL = strSQL & " FROM NF_NOTA "
	strSQL = strSQL & " WHERE COD_NF = " & intCodNF
	strSQL = strSQL & " AND SYS_DTT_INATIVO IS NULL "
	strSQL = strSQL & " AND SITUACAO = 'NAO_EMITIDA' "
	
	AbreRecordSet objRS1, strSQL, objConn, adLockOptimistic, adOpenDynamic, adUseClient, -1
	
	'-------------------------------------------------------------------
	' 3) Busca dados do modelo da Nota necessários para impressão
	'-------------------------------------------------------------------
	If Not objRS1.eof Then
		strSQL =          " SELECT MODELO_HTML, TIPO, ULT_NUM_NF, NUM_LINHAS, TAM_LINHA " ', ULT_NUM_FORM
		...
		If Not objRS2.eof Then
			If ((GetValue(objRS2, "TIPO") = "PADRAO") Or (GetValue(objRS2, "TIPO") = "EXTENDIDO")) And (GetValue(objRS2, "MODELO_HTML") <> "") And (GetValue(objRS2, "ULT_NUM_NF") <> "") And (GetValue(objRS2, "NUM_LINHAS") <> "") And (GetValue(objRS2, "TAM_LINHA") <> "") Then ' And (GetValue(objRS2, "ULT_NUM_FORM") <> "")
				...
				End If
			Else
				strMSG = strMSG & "Dados do Modelo estão incorretos!<br><br>Modelo:" & GetValue(objRS2, "MODELO_HTML") & "<br>Tipo:" & GetValue(objRS2, "TIPO") & "<br>Ult NumNF:" & GetValue(objRS2, "ULT_NUM_NF") & "<br>Num Linhas:" & GetValue(objRS2, "NUM_LINHAS") & "<br>Tam Linha:" & GetValue(objRS2, "TAM_LINHA") '<br>Ult Num Form:" & GetValue(objRS2, "ULT_NUM_FORM")
			End If
		Else
		...
		'-------------------------------------------------------------------------
		' 4) Grava nessa parte o NumNF utilizado para já ficar como utilizado
		'    Os problemas de NF duplicada deve ser resolvidos com essa troca,
		'    antes essa atualização era lá no final
		'------------------------------------------------------ Clv 18/04/2011 ---
		IF (GetValue(objRS2, "ULT_NUM_NF")<>"") THEN 
		  strNUM_NF = CDbl(GetValue(objRS2, "ULT_NUM_NF")) + 1
		ELSE
		  strNUM_NF = 0
		END IF				
		...
		strSQL = " UPDATE CFG_NF SET ULT_NUM_NF = '" & strNUM_NF & "' WHERE COD_CFG_NF = " & GetValue(objRS1, "COD_CFG_NF")
		...
		'-----------------------------------------------
		' 8) Busca os itens da Nota e põe nos arrays
		'-----------------------------------------------
		If strMSG = "" Then
		...
		'----------------------------------------------------------------
		' 10) Na montagem que é gravada em arquivo sai o número da NF
		'----------------------------------------------------------------
		If strMSG = "" Then
		...
		'----------------------------------------------------------
		' 11) Grava arquivo da Nota gerado a partir do modelo
		'----------------------------------------------------------
		If strMSG = "" Then
		...
		If strMSG = "" Then
			'----------------------------------------
			' 12) Atualiza Nota e marca como emitida
			'----------------------------------------
			...
			If Err.Number <> 0 Then
				set objRSCT = objConn.Execute("rollback")
				Mensagem "modulo_FIN_NF.Print B: " & Err.Number & " - "& Err.Description , DEFAULT_LOCATION, 1, True
				Response.End()
			else
				set objRSCT = objConn.Execute("commit")
			End If
			...
			'	If Err.Number <> 0 Then
			'		set objRSCT = objConn.Execute("rollback")
			'		Mensagem "modulo_FIN_NF.Print C" & strNUM_NF & ": " & Err.Number & " - "& Err.Description , DEFAULT_LOCATION, 1, True
			'		Response.End()
			'	else
			'		set objRSCT = objConn.Execute("commit")
			'	End If
			...
			'----------------------------------------------------------------------
			' 13) Atualiza Conta a Receber que tiver vínculo com a nota
			'----------------------------------------------------------------------
