...
<% VerificaDireito "|DEL|", BuscaDireitosFromDB("modulo_MB_REVISTA", Request.Cookies("VBOSS")("ID_USUARIO")), true %>
...
' -------------------------------------------------------------------------------
   ...
   If strCODIGO <> "" Then
	  ...
	  strSQL = " SELECT * FROM mb_revista WHERE COD_REVISTA = " & strCODIGO
...
<body>
<%=athBeginDialog(WMD_WIDTH, "Revista - Dele&ccedil;&atilde;o")%>
<% for Idx = 0 to objRS.fields.count - 7 'NÃO TRAZER TODOS OS DADOS %> 
	...
	<div class="form_bypass">
		<% if(objRS.Fields(Idx).name = "IMG")then %>
			<img src='../upload/<%=Request.Cookies("VBOSS")("CLINAME")%>/MB_REVISTA/<%=GetValue(objRS,"IMG")%>' width='160' />
		<% else %>
