...
<% VerificaDireito "|DEL|", BuscaDireitosFromDB("modulo_CLIENTE", Request.Cookies("VBOSS")("ID_USUARIO")), true %>
...
<%
 ...
 Const auxAVISO  = "dlg_warning.gif:ATENÇÃO!Você está prestes a remover o registro <br> acima visualizado. Para confirmar clique no botão [ok], para desistir clique em [cancelar]."
   ...
   If strCODIGO <> "" Then
	  ...
	  strSQL = "SELECT * FROM ENT_CLIENTE WHERE COD_CLIENTE = " & strCODIGO
...
<body>
<%=athBeginDialog(WMD_WIDTH, "Cliente - Dele&ccedil;&atilde;o")%>     
<% for Idx = 0 to objRS.fields.count - 33  'NÃO QUIZ EXIBIR TODOS OS DADOS... %>
