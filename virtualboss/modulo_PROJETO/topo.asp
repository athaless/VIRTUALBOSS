...
function ExecAcao(pr_form, pr_input) {
	...
	if (form.value=="BACKLOG") { document.form_backlog.submit(); }
	form.value='';
}
...
<form name="form_backlog" method="get" target="vbNucleo" action="../modulo_PROJETO_BACKLOG/default.htm"></form>
...
<tr> 
 	<td width="1%" class="top_menu" style="background-image:url(../img/Menu_TopBgLeft.jpg); vertical-align:top; padding:10px 0px 0px 10px;  border-collapse:collapse;">
	    <b><!-- <a href="Help.htm" target="vbMainFrame" title="sobre este módulo...">[?]&nbsp;</a>-->Projetos</b><br>
		<%=montaMenuCombo("form_acoes","selNome","width:90px","ExecAcao(this.form.name,this.name);","INSERIR:INSERIR;BACKLOG:BACKLOG")%>
	</td>
	...
	<td width="98%" class="top_filtros" style="background-image:url(../img/Menu_TopBgRight.jpg); vertical-align:bottom; padding:0px 5px 5px 0px; margin:0px; text-align:right; border:none; border-collapse:collapse;">
		<div class="form_line">
			<form name="form_principal" id="form_principal" method="get" action="<%=CFG_MAIN_GRID%>" target="vbMainFrame">
				<div class="form_label_nowidth">Cód:</div><input name="var_cod_projeto" type="text" class="edtext" style="width:30px;">
				<div class="form_label_nowidth">Título:</div><input name="var_titulo" type="text" class="edtext">
				<div class="form_label_nowidth">Descrição:</div><input name="var_descricao" type="text" class="edtext">				
