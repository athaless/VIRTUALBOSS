...
<%
 ...
 Dim auxNUM_VOTOS , auxTOT_VOTOS, strID_USUARIO, strFlagTpView
 Dim i, msgERRO, aux

 strCOD_ENQUETE = Request("var_chavereg")
 strID_USUARIO  = Request.Cookies("VBOSS")("ID_USUARIO")
 strFlagTpView  = ucase(Request("var_tpshow")) 'TUDO, OBJETIVA, SUBJETIVA
 if (strFlagTpView="") then
	strFlagTpView = "OBJETIVA"
 ElseIf (strFlagTpView="TUDO") then
	strFlagTpView = "%"
 End If
 ...
 strSQL = " SELECT  t3.titulo "&_
		  ...
		  "	   LEFT JOIN en_alternativa as t1 ON (t2.cod_questao = t1.cod_questao AND UCASE(t1.tipo) LIKE '" & strFlagTpView & "') "&_
...
If (NOT objRS.EOF) then 
 ...
 aux = ( CInt(getValue(objRS,"QTDE_VOTANTES")) / CInt(getValue(objRS,"QTDE_USUARIOS")) ) * 100
'response.Write(aux & " quorum: " & CInt(getValue(objRS,"quorum")))
 IF ( aux < CInt(getValue(objRS,"quorum")) )  THEN
	 msgERRO = "Não há quantidade de respostas suficientes para exibição das parciais:<small> " & aux & " (menos de "& getValue(objRS,"quorum") &"%)</small>."
 End If
		...
		END IF  %>
			...
			<% If uCase(getValue(objRS,"TIPO")) = "OBJETIVA" then %>
                <div style="padding: 0px 0px 0px 30px; border:0px solid #C90; text-align:left;">
                    ...
                    %>
                    <img src='../img/barra_enquete.jpg' width="<%=FormatNumber((auxNUM_VOTOS/auxTOT_VOTOS) * 360,0)%>" height='7'>
                    &nbsp;<%=FormatNumber((auxNUM_VOTOS/auxTOT_VOTOS) * 100, 2)%>%
                    <%  End If %>
                </div>		
			<% Else %>
                <div style="padding: 0px 0px 0px 30px; border:0px solid #C90; text-align:left;">
                    <%=Replace(getValue(objRS,"RESPOSTAS")," | | ","<hr>")%>
                </div>		
			<% End IF %>
		<%
