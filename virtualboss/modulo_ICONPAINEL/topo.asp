...
function ExecAcao(){
	...
	if (form.selNome.value=='INSERIR')  
		window.open("insert.asp","Atalhos","width=530, height=730, left=30, top=30, scrollbars=1, status=0");
	...
	<td width="98%" class="top_filtros" style="background-image:url(../img/Menu_TopBgRight.jpg); vertical-align:bottom; padding:0px 5px 5px 0px; margin:0px; text-align:right; border:none; border-collapse:collapse;">
		<div class="form_line">
			<form name="form_principal" id="form_principal" method="get" action="<%=CFG_MAIN_GRID%>" target="vbMainFrame">
				<div class="form_label_nowidth">Rótulo:</div><input name="var_rotulo" type="text" size="20" class="edtext">			
					...
					<select name="var_id_usuario" class="edtext_combo" style="width:100px" >
						<%
							if ((Request.Cookies("VBOSS")("GRUPO_USUARIO")="MANAGER") or (Request.Cookies("VBOSS")("GRUPO_USUARIO")="ADMIN") or (Request.Cookies("VBOSS")("GRUPO_USUARIO")="SU")) then 
								...
								montaCombo "STR","SELECT DISTINCT USUARIO_HORARIO.ID_USUARIO FROM USUARIO_HORARIO, USUARIO WHERE USUARIO.ID_USUARIO = USUARIO_HORARIO.ID_USUARIO AND DT_INATIVO is NULL ORDER BY USUARIO.ID_USUARIO", "ID_USUARIO", "ID_USUARIO", Request.Cookies("VBOSS")("ID_USUARIO")
							else
