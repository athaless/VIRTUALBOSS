...
<% 
  ...
  CFG_MAIN_GRID     = "main.asp"  					        'Define o action dos filtros (topo.asp) - main.asp ou preload.asp
...
Function MontaDbUserName(prDefault, prDataBase)
	...
	auxStr2 = lcase(server.mappath("/"))
   	...
   	IF (instr(auxStr1,"/web_systems/") > 0) then ' SE ESTIVER NA ATHENAS 	
	  auxUSERdb = prDefault
   	ELSE
...
Function FindUploadPath
  ...
  auxStr2 = lcase(server.mappath("/"))
  If (instr(auxStr1,"/web_systems/") > 0) Then ' SE ESTIVER NA ATHENAS 	
    auxStr2 = auxStr2 & "\web_systems\virtualboss" & CFG_DIR
  Else
...
Function FindPhysicalPath(pr_pasta)
  ...
  auxStr2 = lcase(server.mappath("/"))
  If (instr(auxStr1,"/web_systems/") > 0) Then ' SE ESTIVER NA ATHENAS 	
    auxStr2 = auxStr2 & "\web_systems" & pr_pasta
  Else
...
Function FindLogicalPath()
  ...
  If (instr(auxmappath,"/web_systems/") > 0) Then ' SE ESTIVER NA ATHENAS 	
    auxmappath = "http://" & Request.ServerVariables("HTTP_HOST") & "/web_systems/virtualboss/virtualboss"
  Else
...
'	else
'	  auxmappath = "http://" & Request.ServerVariables("HTTP_HOST") & "/web_systems/virtualboss/virtualboss" 'ATHENAS
'	end if
'  else
'  	'LOCAWEB v1 Ou LOCAWEB v2 Ou PLUGIN
