...
' SESSION ou COKIE
function replaceParametersSessionCokie(prString)
	...
	if (mixPos>=1) then
		while (mixPos>=1) AND (not isEmpty(mixPos))
			...
			strAuxSQL = replace(retValue, strIndex, Request.Cookies("VBOSS")(strIndexS), 1)
			retValue  = strAuxSQL
			mixPos    = instr(retValue,"{")
		wend
	end if
	replaceParametersSessionCokie = retValue
end function
...
End Function
 ...
 Function ExibePROJETO(prCODIGO, prTITULO, prFASE_ATUAL)
	...
	strSAIDA = strSAIDA & "		<td width='16' align='center'><a href=""Javascript: MyShowArea('prj_" & prCODIGO & "', 'icon_prj_" & prCODIGO & "');""><img src='../img/BulletMenos.gif' border='0' align='absmiddle' name='icon_prj_" & prCODIGO & "' id='icon_prj_" & prCODIGO & "'></a></td>" & vbNewLine
 ...
 Function ExibeBS(prCODIGO, prTITULO, prCOD_CLIENTE, prCLIENTE, prRESPONSAVEL, prSITUACAO, prDT_INI, prDT_FIM, prPREV_HORAS, prHORAS, prNA_EQUIPE)
	...
	strSAIDA = strSAIDA & " <td width='16' align='center'><a href=""Javascript: MyShowArea('ativ_" & prCODIGO & "', 'icon_ativ_" & prCODIGO & "');""><img src='../img/BulletMenos.gif' border='0' align='absmiddle' name='icon_ativ_" & prCODIGO & "' id='icon_ativ_" & prCODIGO & "'></a></td>" & vbNewLine
	...
	If prCODIGO <> "" And prTITULO <> "" Then
		...
		strSAIDA = strSAIDA & " <td width='160' style='text-align:right;' valign='top' class='arial11'></td>" & vbNewLine 'de " & prDT_INI & " a " & prDT_FIM & "
		...
		'Não estamos mais exibindo essas informações porque estavam tornando a consulta lenta, gerando problemas 
		'quando o usuário tivesse muita coisa para ver. Quando, no futuro, os campos estiverem na atividade e 
		'forem calculados por trigger iremos exibir os dados abaixo 
...
'		strSAIDA = strSAIDA & "	</div>" & vbNewLine
 ...
 Function ExibeTODOLIST(prCODIGO, prTITULO, prRESPONSAVEL, prEXECUTOR, prCATEGORIA, prDT_INI, prHR_INI, prHORAS, prSITUACAO, prPRIORIDADE, prCOD_BS, prCABECALHO)
	...
	If prCABECALHO Then
		...
		strSAIDA = strSAIDA & "	<td width='16'></td>" & vbNewLine
		strSAIDA = strSAIDA & "	<td width='60' nowrap='nowrap'>Data</td>" & vbNewLine
		strSAIDA = strSAIDA & "	<td width='15' nowrap='nowrap'></td>" & vbNewLine
	...
	strSAIDA = strSAIDA & "	<td width='60' valign='top'>"
	If strUSER_ID = LCase(prEXECUTOR) Then strSAIDA = strSAIDA & "<strong>"
	strSAIDA = strSAIDA & LCase(prEXECUTOR)
	If strUSER_ID = LCase(prEXECUTOR) Then strSAIDA = strSAIDA & "</strong>"
	strSAIDA = strSAIDA & "</td>" & vbNewLine
...
Function MontaSQLBuscaPAT(prIN_DATA, prIN_ID_USUARIO, prIN_VIEW, prLIMIT)
	...
	IF (lcase(prIN_VIEW) = "exec_equipe") THEN
		...
		'Não estamos mais trazendo essas informações porque estavam tornando a consulta lenta, gerando problemas 
		'quando o usuário tivesse muita coisa para ver. Os campos serão mantidos na consulta porque no futuro 
		'iremos colocá-los na atividade e serão calculados via trigger. Assim, bastará lê-los nessa parte.
		...
		auxSQL = auxSQL & " WHERE ( (TL.SITUACAO <> 'FECHADO') AND (TL.SITUACAO <> 'CANCELADO') )"
		auxSQL = auxSQL & " AND TL.PREV_DT_INI <= '" & prIN_DATA & "' " 
		auxSQL = auxSQL & " AND (((BS.TIPO <> 'MODELO' OR BS.TIPO IS NULL) AND TL.ID_ULT_EXECUTOR = '" & prIN_ID_USUARIO & "') "
		auxSQL = auxSQL & "      OR ((SELECT DISTINCT COD_BOLETIM FROM BS_EQUIPE WHERE ID_USUARIO = '" & prIN_ID_USUARIO & "' AND BS_EQUIPE.COD_BOLETIM = TL.COD_BOLETIM) IS NOT NULL) AND (BS.TIPO <> 'MODELO') ) "

		auxSQL = auxSQL & " ORDER BY PRJ.TITULO, PRJ.COD_PROJETO, BS.TITULO, BS.COD_BOLETIM, TL.PREV_DT_INI, TL.TITULO, TL.COD_TODOLIST "
...
' ---------------------------------------------------------------------------------------------
Function RetNumbers(prNum)
    Dim Tamanho, RESULTADO, A
    ...
    Tamanho = Len(Trim(prNum))
    
    For A = 1 To Tamanho
        If Asc(Mid(prNum, A, 1)) >= 48 And Asc(Mid(prNum, A, 1)) <= 57 Then
            RESULTADO = RESULTADO + Mid(prNum, A, 1)
        End If
    Next
    RetNumbers = RESULTADO
End Function
