...
<body>
<%=athBeginDialog(WMD_WIDTH, "Plano de Conta - Alteração")%>
<form name="form_update" action="InsUpd_Exec.asp" method="post">
	...
	<br><div class="form_label">Descrição:</div><input name="var_descricao" value="<%=GetValue(objRS,"DESCRICAO")%>" type="text" maxlength="250" style="width:250px;">
	<br><div class="form_label">DRE:</div><input name="var_dre" id="var_dre" type="radio" value="True" class="inputclean" <% If GetValue(objRS,"DRE") <> "0" Then Response.Write(" checked") %>>Sim<input name="var_dre" id="var_dre" type="radio" value="False" class="inputclean" <% If GetValue(objRS,"DRE") = "0" Then Response.Write(" checked") %>>Não&nbsp;<span class="texto_ajuda">Deve aparecer no Demonstrativo do Relatório de Exercício</span>
