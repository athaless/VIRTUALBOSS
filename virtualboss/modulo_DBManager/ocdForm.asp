...
%><!--#INCLUDE FILE=ocdForm_Lang.asp--><%
	...
	End If
		...
		If Not IsObject(ADOConnection) Then
			...
			If ADOMode = 0 Then
				If FormMode <> "Salvar" And Not ((Request.Form("ocdEditDelete") <> "" Or Request.QueryString("ocdEditDelete") <> "") And request("ocdEditconfirm") <> "") Then	'nothing to update, use read only connection
		...
		ElseIf FormMode = "Salvar" Then
			If (AllowEdit And EditMode = "Edit") Then
				...
				Call ADORecordset.open(strSQL, ADOConnection, 3, 3)
				...
				End If
				If err.number <> 0 Then
					FormStatus = FormStatus & err.description & " (salvar-upd) "
				Else
					If ADORecordset.eof Then
						EditMode = "NotFound"
						FormStatus = FormStatus & HTMLNotFound
					ElseIf CallValidateBeforeUpdate Then
						Call ocdValidateBeforeUpNow()
			...
			ElseIf (AllowAdd And EditMode = "Add") Then
				...
				End If
				If err.number <> 0 Then
					FormStatus = FormStatus & err.description & "(salvar-ins)"
			...
			If FormStatus = "" And InvalidFields = "" Then
				For Each fmRequest In Request.Form
										...
										Select Case fldTemp.Type
											...
											Case Else
												...
												Else
													...
													End If
													If err.number <> 0 Then
														FormStatus    = FormStatus & HTMLUpdateFieldFails & mid(fmRequest, 6) & ": " & err.description & " (lin 1009)<BR>"
												...
												Case Else
													...
													Else
														If fldTemp.Type = 135 And IsDate(Request.Form(fmRequest)) And DatabaseType = "MySQL" Then
														    ...
														    'AQUI DEBUG
															'DIM auxDateF, auxStr
															'auxDateF = CDate(Request.Form(fmRequest))
															'auxStr   = Year((auxDateF)) & "-" & Month((auxDateF)) & "-" & Day((auxDateF))
															'fldTemp.Value = "2009-12-12" '"12/12/2009"
															'Response.write (auxDateF & "  <==>  " & auxStr & "  <==>  " & fldTemp.Value & " ["& fldTemp.Name & "]")
															'response.end
														...
														Select Case Err.number
															Case 0
															Case -2147217887
																FormStatus = FormStatus & " " & HTMLUpdateFieldFails & " " & mid(fmRequest, 6) & ": " & HTMLOLEError & " (lin 1153)<BR>"
																InvalidFields = InvalidFields & mid(fmRequest, 6) & ";"
																err.clear()
															Case Else
																FormStatus = FormStatus & " " & HTMLUpdateFieldFails & " " & mid(fmRequest, 6) & ": " & err.description & " (lin 1157)<BR>"
																InvalidFields = InvalidFields & mid(fmRequest, 6) & ";"
																err.clear()
														End Select
													End If
					...
					If err.number <> 0 Then
						FormStatus = FormStatus & HTMLStatusFieldError & mid(fmRequest, 6) & ": " & err.description & " (lin 1169)<BR>"
						InvalidFields = InvalidFields & mid(fmRequest, 6) & ";"
						err.clear()
					End If
			...
			If InvalidFields = "" And FormStatus = "" Then
				...
				Else
					' Pelo DEVGURU não existe o metodo UpNow, mas sim Update no ADORecordSet, 
					' então trocamos e parece funcionar ok para MySQL agora
					' ----------------------------------------------- 28/01/2010 Aless/Leandro --
					'ADORecordset.UpNow()
					ADORecordset.Update()
				End If

				If err.number <> 0 Then
					FormStatus = FormStatus & err.description & " (lin 1191)<BR>"
					ADORecordset.CancelUpNow()
					err.clear()
					'On Error GoTo 0
				Else
					If EditMode = "Add" Then
						If EditMode = "Add" And FormMode = "Salvar" And Not DatabaseType = "Oracle" And DatabaseType <> "MySQL" Then  'redirect to newly added record
