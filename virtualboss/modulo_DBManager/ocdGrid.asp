...
'		response.write blncomputable
		...
		If UCase(GridMode) = "PROCESS" Or UCase(GridMode) = "BUSCA" Then
...
'				ADORecordset.Filter = SQLWhereExtra
		...
		If UCase(GridMode) = "EXPORT" And UCase(Request.QueryString("ocdExportFormat_" & GRIDID)) = "XML" Then
			...
			If SQLRecordCount > GridMaxRecordsDisplay Then
				Response.Write("<xml><warning>Limite de Exibição Excedido</warning></xml>")
			Else
		...
		Select Case UCase(strTemplate)
			Case "QUICK", "", "GRID"
				...
				If intLoopSize = GridMaxRecordsDisplay AND UCase(GridMode) <> "BROWSE" Then
					Response.Write("<p>Limite de Exibição Excedido - First " & GridMaxRecordsDisplay & " Records Displayed</p>")
				End if
...
Response.Write(varValue)
					...
					If AllowDelete And (intSQLSelectIDPos <> -1 Or ((SQLSelectPK <> ""))) Then
						tmpSQLPKString = ""
						Response.Write("<a href=""" & FormDelete & "?" & strPKqstTemp & "&amp;ocdEditDelete=Deletar"" " & HTMLAttribDeleteLink & " >" & HTMLDeleteLink & "</a>")
						If AllowMultiDelete Then
							Response.Write("<input type=""checkbox"" name=""sqlid"" value=""" & Server.URLEncode(arrGridData(intSQLSelectIDPos, intRowCount - 1)) & """>")
						End If
					Else
	...
	Public Sub DisplayKeywordSearch()
		...
		next
		Response.Write("<input name=""ocdKeyword_" & GRIDID & """ value=""" & Server.htmlencode(Request.QueryString("ocdKeyword_" & GRIDID)) & """ size=""12"" maxlength=""255""> " & "<input name=""ocdFind_" & GRIDID & """ type=""submit"" value=""Busca"" class=""submit"">" & "<input name=""ocdFindBrowse"" type=""hidden"" value=""yes"">")
	...
	Private Sub DisplayGridButtons()
		...
		For Each eleShowGridButtons In arrShowGridButtons
			...
			Select Case UCase(arrGRIDBE(0))
				Case "BUSCA"
					Response.Write(" <a href=""" & FormSearch & "?ocdGridMode_" & GRIDID & "=Busca")
					For Each qstTemp In Request.QueryString
	...
	Private Sub DisplaySearch(ByVal strGridID)
		...
		End If
		Response.Write("<p><table><tr><td align=""right"" valign=""top"" nowrap><span class=""FieldName"">Ordena Por:</span></td><td valign=""top"" nowrap colspan=3>")
		...
		Next
		Response.Write("</td></tr><tr><td align=""right"" nowrap><span class=""FieldName"">Resultados Por Página:</span></td><td nowrap colspan=""3"">")
