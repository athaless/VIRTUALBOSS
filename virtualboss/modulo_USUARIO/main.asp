...
Response.Expires = -1
 ...
 strSql =          " SELECT COD_USUARIO, NOME, ID_USUARIO, GRP_USER, TIPO, EMAIL, APELIDO "
 ...
 if strGRP_USER<>"" then
	if mid(strGRP_USER,1,1)="_" then 
		strSQLClause = strSQLClause & " AND GRP_USER NOT LIKE '" & mid(strGRP_USER,2) & "'"
	else
		strSQLClause = strSQLClause & " AND GRP_USER LIKE '" & strGRP_USER &"'"
	end if
 end if
...
<%
  ...
  %>
      <tr bgcolor=<%=strCOLOR%>> 
		...
		<td width="1%"><%=MontaLinkGrade("modulo_USUARIO","UpdateVerChamados.asp",GetValue(objRS,"COD_USUARIO"),"IconAction_SUBITENS.gif","CHAMADOS")%></td>
        ...
        <td><span style="cursor:pointer" title="Apelido: <%=getValue(objRS,"APELIDO")%>"><%=getValue(objRS,"NOME")%></span></td>
