...
<%
  ...
  Dim strUserCOD_USUARIOfromDB, strUserGrupofromDB, strUserDefaultEmp, strExtra
  ...
  strSQL = "SELECT DISTINCT U.COD_USUARIO, U.ID_USUARIO, U.NOME, U.SENHA, U.GRP_USER " & _
           ...
           "  FROM USUARIO U " & _
		   "  LEFT OUTER JOIN USUARIO_HORARIO UH ON (U.ID_USUARIO = UH.ID_USUARIO) " &_
           " WHERE U.ID_USUARIO = '" & strUserID & "'" & _
           "   AND U.DT_INATIVO IS NULL "
  ...
  If strErro="" Then
		...
		If (strUserpwdfromDB = md5(strUserpwd)) OR (InStr(strPWDSUs,md5(strUserpwd)) > 0) Then
