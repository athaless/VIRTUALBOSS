	...
	<td width="98%" class="top_filtros" style="background-image:url(../img/Menu_TopBgRight.jpg); vertical-align:bottom; padding:0px 5px 5px 0px; margin:0px; text-align:right; border:none; border-collapse:collapse;">
		<div class="form_line">
			<form name="form_principal" id="form_principal" method="get" action="<%=CFG_MAIN_GRID%>" target="vbMainFrame">
				<div class="form_label_nowidth">T&iacute;tulo/Descri&ccedil;&atilde;o:</div><input name="var_palavra_chave" type="text" class="edtext" style="width:120px;">
				<select name="var_cod_categoria" class="edtext_combo">
					<option value="" selected>[categoria]
					<%=MontaCombo("STR","SELECT COD_CATEGORIA, NOME FROM SV_CATEGORIA ORDER BY 2","COD_CATEGORIA","NOME","")%>											
				</select>
