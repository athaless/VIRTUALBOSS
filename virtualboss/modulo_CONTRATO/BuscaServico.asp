...
function Retorna(valor1, valor2, valor3, valor4) { 
    <%
		If strINPUT1 <> "" Then Response.write "window.opener.document." & strFORM & "." & strINPUT1 & "." & "value = valor1;"
     	If strINPUT2 <> "" Then Response.write "window.opener.document." & strFORM & "." & strINPUT2 & "." & "value = valor2;"	
    	If strINPUT3 <> "" Then Response.write "window.opener.document." & strFORM & "." & strINPUT3 & "." & "value = valor3;"	
    	If strINPUT4 <> "" Then Response.write "window.opener.document." & strFORM & "." & strINPUT4 & "." & "value = valor4;"	
	%>	
...
%>
	<tr bgcolor="<%=strCOLOR%>" style="cursor:hand;" onMouseOver="this.style.backgroundColor='#FFCC66';" onMouseOut="this.style.backgroundColor='';" onClick="Retorna('<%=GetValue(objRS,"COD_SERVICO")%>','<%=GetValue(objRS,"TITULO")%>','<%=GetValue(objRS,"VALOR")%>','<%=GetValue(objRS,"ALIQ_ISSQN")%>');">
		<td></td>
