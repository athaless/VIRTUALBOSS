...
<%
 ...
 if (strANO <> "") and (strCOD_CONTA <> "") then
	...
	strSQL = "SELECT PGR.COD_CONTA_PAGAR_RECEBER AS CODIGO " 		&_
				...
				"	,if(PGR.PAGAR_RECEBER,'SAIDA','ENTRADA') AS OPERACAO " &_
				"	,'TITULOS' AS MODULO " 							&_
				"	,PGR.TIPO " 									&_
	...
	if strDT_INI <> "" And strDT_FIM <> "" then 
		strSQL = strSQL & " AND ORD.DT_LCTO BETWEEN '" & PrepDataBrToUni(strDT_INI, False) & "' AND '" & PrepDataBrToUni(strDT_FIM, False) & "' "
	else	
		strSQL = strSQL & " AND Year(ORD.DT_LCTO)>=" & strANO
	end if
	...
	If strCOD_CONTA <> "" Then strSQL = strSQL & " AND CTA2.COD_CONTA = " & strCOD_CONTA
	if strDT_INI <> "" And strDT_FIM <> "" then
		strSQL = strSQL & " AND LCT.DT_LCTO BETWEEN '" & PrepDataBrToUni(strDT_INI, False) & "' AND '" & PrepDataBrToUni(strDT_FIM, False) & "' "
	else
		strSQL = strSQL & " AND Year(LCT.DT_LCTO)>=" & strANO
	end if
	...
	If (strDT_INI <> "" Or strDT_FIM <> "" Or strCOD_CONTA <> "") And strCODIGO = "" And strTIPO = "" Then 
		...
		if strDT_INI <> "" And strDT_FIM <> "" then
			strSQL = strSQL & " AND LTF.DT_LCTO BETWEEN '" & PrepDataBrToUni(strDT_INI, False) & "' AND '" & PrepDataBrToUni(strDT_FIM, False) & "' "
		else
			strSQL = strSQL & " AND Year(LTF.DT_LCTO)>=" & strANO
		end if
	...
	'Passamos a usar a procedure para trazer os dados; mantemos o SQL anterior para caso 
	'precisemos desativar a proc - by Clv/Aless - 13/01/2010
	'A partir daqui estamos reescrevendo o SQL para usar procedure ----------------------------------------------------------------------------
	'If strDT_INI <> ""           Then strPARAM1 = "'" & PrepDataBrToUni(strDT_INI, False) & "'" Else strPARAM1 = "NULL" End If
	'If strDT_FIM <> ""           Then strPARAM2 = "'" & PrepDataBrToUni(strDT_FIM, False) & "'" Else strPARAM2 = "NULL" End If
	'If strANO <> ""              Then strPARAM3 = strANO                                        Else strPARAM3 = "NULL" End If
	'If strCODIGO <> ""           Then strPARAM4 = "'" & strCODIGO & "'"                         Else strPARAM4 = "NULL" End If
	'If strTIPO <> ""             Then strPARAM5 = "'" & strTIPO & "'"                           Else strPARAM5 = "NULL" End If
	'If strCOD_CONTA <> ""        Then strPARAM6 = strCOD_CONTA                                  Else strPARAM6 = "NULL" End If
	'If strCOD_PLANO_CONTA <> ""  Then strPARAM7 = strCOD_PLANO_CONTA                            Else strPARAM7 = "NULL" End If
	'If strCOD_CENTRO_CUSTO <> "" Then strPARAM8 = strCOD_CENTRO_CUSTO                           Else strPARAM8 = "NULL" End If
	
	'strSQL = " CALL sp_busca_lctos_gerais( " & strPARAM1 & ", " & strPARAM2 & ", " & strPARAM3 & ", " & strPARAM4 &_
	'         "                           , " & strPARAM5 & ", " & strPARAM6 & ", " & strPARAM7 & ", " & strPARAM8 & ")"
	'------------------------------------------------------------------------------------------------------------------------------------------
