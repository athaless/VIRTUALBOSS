...
strRETORNO3 = GetParam("var_retorno3")
strPALAVRA_CHAVE = GetParam("var_palavra_chave")

AbreDBConn objConn, CFG_DB 

 '-------------------------------------------------------------------
 ' Função recursiva que carrega numa TreeView os planos de contas
 '-------------------------------------------------------------------	
 ...
 Dim strTexto1, strTexto2
   ...
   strSQLLocal = " SELECT COD_PLANO_CONTA AS CODIGO, NOME, COD_REDUZIDO, DT_INATIVO FROM FIN_PLANO_CONTA " 
   if pr_codigo_menu <> "" then
   	  strSQLLocal = strSQLLocal & " WHERE COD_PLANO_CONTA_PAI = " & pr_codigo_menu
   else
      if pr_codigos_raiz <> "" then
     	strSQLLocal = strSQLLocal & " WHERE COD_PLANO_CONTA IN (" & pr_codigos_raiz & ") "
      else
     	strSQLLocal = strSQLLocal & " WHERE COD_PLANO_CONTA_PAI IS NULL " 
      end if
   end if
   if pr_codigos_caminho <> "" then strSQLLocal = strSQLLocal & " AND COD_PLANO_CONTA IN (" & pr_codigos_caminho & ") "
   ...
   While not objRSLocal.EOF
	  ...
	  objChild.ChildNodes.Add(objTree.CreateNode(strTexto2,"Javascript:GuardaInfo('" & GetValue(objRSLocal,"CODIGO") & "', '" & GetValue(objRSLocal,"COD_REDUZIDO") & "', '" & strTexto1 & "')",""))
 ...
 '-------------------------------------------------------------------------------------------------
 ' Função recursiva que retorna os códigos dos planos de contas que se enquadraram na pesquisa
 '-------------------------------------------------------------------------------------------------	
 ...
 Dim strCOD_ITEM, strCOD_PAI
   ...
   strSQLLocal =               " SELECT COD_PLANO_CONTA, COD_PLANO_CONTA_PAI, NOME, COD_REDUZIDO, DT_INATIVO " 
   strSQLLocal = strSQLLocal & " FROM FIN_PLANO_CONTA " 
   ...
   While not objRS1.EOF
		strCOD_ITEM = GetValue(objRS1,"COD_PLANO_CONTA")
   		strCOD_PAI  = GetValue(objRS1,"COD_PLANO_CONTA_PAI")
		...
		Else
			While strCOD_PAI <> ""
				strSQL = " SELECT COD_PLANO_CONTA_PAI, COD_PLANO_CONTA FROM FIN_PLANO_CONTA WHERE COD_PLANO_CONTA = " & strCOD_PAI
				Set objRS2 = objConn.Execute(strSQL)
				If Not objRS2.Eof Then
					strCOD_ITEM = GetValue(objRS2,"COD_PLANO_CONTA")
					strCOD_PAI  = GetValue(objRS2,"COD_PLANO_CONTA_PAI")
 ...
 If strPALAVRA_CHAVE = "%" Then
	objTree.AddNode("<div class='texto_ajuda'>" & ReturnCodigo("Plano de Conta") & "</div>")
...
<!--
function GuardaInfo(pr_codigo, var_cod_reduzido, pr_nome) { 
	document.formretorno.var_codigo.value = pr_codigo; 
	document.formretorno.var_hierarquia.value = '.../' + pr_nome; 
	document.formretorno.var_cod_reduzido.value = var_cod_reduzido;
}
...
function Ok() {
	...
	if ((document.formretorno.var_codigo.value != '') && (document.formretorno.var_hierarquia.value != '') && (document.formretorno.var_cod_reduzido.value != '')) {
		if (var_form != '') {
		...
		objOption.innerText	= document.formretorno.var_cod_reduzido.value + " " + (document.formretorno.var_hierarquia.value).replace(".../","");
		...
		<td width="95%" background="../img/Menu_TopBgRight.jpg" align="right">
			<div style="padding-top:20px;padding-right:3px;"> 
				<table height="30" cellpadding="0" cellspacing="0" border="0">
					<form name="FormBusca" method="post" action="BuscaPlanoConta.asp">
					...
					<input name="var_retorno3" type="hidden" value="<%=strRETORNO3%>">
					<tr>
	...
	<div style="padding-top:10px;">
		...
		<tr>
			<td width="99%" align="right" valign="top">
				<form name="formretorno" action="" method="post">
					...
					<input name="var_cod_reduzido" id="var_cod_reduzido" type="hidden">
				</form>
			</td>
			<td width="1%" valign="top">
			<div style="padding-right:40px;">
				<a href="#" onClick="javascript:Ok();"><img src="../img/BtOk.gif" border="0" hspace="6"></a>
			</div>
			</td>
		</tr>
		</table>
