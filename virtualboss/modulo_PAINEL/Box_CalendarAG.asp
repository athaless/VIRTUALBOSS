...
<%
  ...
  strSQL = " SELECT DISTINCT Day(PREV_DT_INI) AS DIA " &_
		   "   FROM AG_AGENDA WHERE SITUACAO <> 'FECHADO' " &_
		   "    AND Month(PREV_DT_INI) = " & CurrentMonth &_
		   "    AND Year(PREV_DT_INI) = " & CurrentYear
			
	if strGRUPO_USUARIO<>"MANAGER" then
		strSQL = strSQL & "    AND (ID_CITADOS LIKE '%;" & strUserExec & ";%' OR ID_RESPONSAVEL = '" & strUserExec & "')"
	else
		if strUserExec<>"" then
			strSQL = strSQL & "    AND (ID_CITADOS LIKE '%;" & strUserExec & ";%' OR ID_RESPONSAVEL = '" & strUserExec & "')"
		end if
...
<table width=150 height="20" border="0" align="center" cellpadding="0" cellspacing="1">
  <tr>
    <td align="center" width="20"><a href="Box_CalendarAG.asp?dtAtual=<%= DateAdd("m",-1, EventDate)%>&var_exec=<%=strUserExec%>" class="calendario"><img src="../img/seta_prev.gif" border="0"></a></td>
	 <td align="center"><font><b><%= CurrentMonthName %>/<%= CurrentYear %></b></font></td>
    <td align="center" width="20"><a href="Box_CalendarAG.asp?dtAtual=<%= DateAdd("m",1,EventDate)%>&var_exec=<%=strUserExec%>" class="calendario"><img src="../img/seta_prox.gif" border="0"></a></td>
  </tr>
</table>  
	 ...
	 %>
        <td height="16" align="right" background="../img/<%=strBGCELL%>">
		  <div style="padding-right:4px;">
		    <a href="painel.asp?var_dia_agenda_selected=<%=CurrentDay%>&var_exec=<%=strUserExec%>" target="_parent" class="corpo_texto_peq"><%=Day(CurrentDay)%></a>
		  </div>
		</td>
     <%
     Else
