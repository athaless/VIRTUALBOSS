<%
	...
	'
	' Remove all public rooms that don't have an owner
	'
	Private Function removePublicRooms()
		...
		For Each roomId In conquerChatRooms
			Set room = getRoom(roomId)
			If (NOT room Is Nothing) Then
				If (room.createdBy = "-1") And (room.tipoPublica = True) Then
					removeRoom(room.id)
				End If
			End If
		Next
	...
	End Function ' // > Private Function removePublicRooms()
...
'				If (StrComp(CStr(message.receiverId), CStr(dstUserId), 1) = 0) Then
	...
	Function setupPublicRooms(strROOMS)
		...
		'objConnDebug.Execute("INSERT INTO SYS_DEBUG (LOG) VALUES ('criando salas:" & strROOMS & "')") 
		...
		If (IsArray(defaultRooms)) Then
			...
			For i = 0 To UBound(defaultRooms)
				...
				If (defaultRooms(i) <> "") Then
					...
					'objConnDebug.Execute("INSERT INTO SYS_DEBUG (LOG) VALUES ('criada sala:" & defaultRooms(i) & "')") 
				End If
			Next
		End If
		Application.UnLock
		
		'objConnDebug.Execute("INSERT INTO SYS_DEBUG (LOG) VALUES ('salas criadas')") 
...
'		End If
	...
	Function getFirstPublicRoom()
		...
		Set getFirstPublicRoom = id
		
	End Function ' // > Function getFirstPublicRoom()
%>