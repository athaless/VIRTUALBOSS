<%
		Dim objConnDebug
		AbreDBConn objConnDebug, CFG_DB
	...
	Function loggedOn()
		...
		'Debugging
		AbreDBConn objConnDebug, CFG_DB
		objConnDebug.Execute("INSERT INTO SYS_DEBUG (LOG) VALUES ('passou por loggedOn SESSIONKEY_USER:" & Session(SESSIONKEY_USER) & " - " & p.id & "') ") 
		FechaDBConn objConnDebug
	End Function
	...
	Function addUser(user)
		...
		'Debugging
		objConnDebug.Execute("INSERT INTO SYS_DEBUG (LOG) VALUES ('passando por addUser:" & user & "') ") 
		...
		'Debugging
		objConnDebug.Execute("INSERT INTO SYS_DEBUG (LOG) VALUES ('passou por addUser:" & user & "') ") 
	...
	Sub logoutUser(userId)
		...
		Call addMessage( _
			user.id, _
			"-1", _
			"<span class=LoggedOut><img src='images/bp.gif' height=9 width=9>&nbsp;" & getMsg("user.logged_off", user.name, PrepData(Now(), True, True)) & "</span><br>" _
		)
	...
	'
	' Adds a new room with specified name, owned by specified user and if is public one.
	' 
	' @param 	roomName 	Name of room to add.
	' @param 	userId 		Id of user owning this room.
	' @param 	tipoPublica True if it's a public one, False if it isn't 
	' @param	tipoCriador	Type of the owner of the room
	' @return 	True if room was successfully created; False otherwise.
	'
	Function addRoom(roomName, userId, tipoPublica, tipoCriador)
		...
		If (getRoomByName(roomName) Is Nothing) Then
			...
			room.tipoPublica = tipoPublica
			room.tipoCriador = tipoCriador 
...
'				If (StrComp(CStr(message.receiverId), CStr(dstUserId), 1) = 0) Then
	...
	Function setupPublicRooms(strROOMS)
		...
		'Debugging
		objConnDebug.Execute("INSERT INTO SYS_DEBUG (LOG) VALUES ('passou por setupPublicRooms:" & countPublicRooms & " - " & strROOMS & "') ") 
		
	End Function ' // > Function setupPublicRooms()
	...
	'Function setupRoom()
	'
	'Criada para permitir criar salas, públicas ou não
	'10/10/2006 by Clv
	Function setupRoom(nameRoom, userId, tipoPublico, tipoCriador) 
		
		'Application.Lock
		If (nameRoom <> "") Then
			Call addRoom(nameRoom, userId, tipoPublico, tipoCriador)
		End If
		'Application.UnLock
		
		'Debugging
		objConnDebug.Execute("INSERT INTO SYS_DEBUG (LOG) VALUES ('passou por setupRoom') ") 
		
	End Function ' // > Function setupRoom()
...
'				End If
		...
		'Debugging
		objConnDebug.Execute("INSERT INTO SYS_DEBUG (LOG) VALUES ('passou po kickInactiveUsers') ") 
...
'		End If
	...
	'
	' The countPublicRooms function returns the number of public rooms in chat
	' 
	Function countPublicRooms()
		...
		countPublicRooms = cnt
		
	End Function ' // > Private Function countPublicRooms()
