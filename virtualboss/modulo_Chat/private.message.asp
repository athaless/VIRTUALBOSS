...
<head>
	...
	<!--
		...
		function sendMessage() {
			...
			if (message == '') {
				executeGetParam('action=refresh,userId=<%= userId %>');
				return;
			}
			
			// add new message to site
			executeGetParam('action=private.refresh,mode=message,message=' + escape(message) + ',userId=<%= userId %>,toUserId=<%= dstUserId %>');
