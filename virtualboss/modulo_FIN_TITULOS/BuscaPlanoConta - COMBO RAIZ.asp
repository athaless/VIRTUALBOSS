...
Sub ExibeLinha(prCOD_PLANO_CONTA, prCOD_REDUZIDO, prNOME, prNIVEL, prDT_INATIVO, prCOLOR)
	Response.Write("<tr bgcolor=" & prCOLOR & " style='cursor:hand;' onMouseOver=""this.style.backgroundColor='#FFCC66';"" onMouseOut=""this.style.backgroundColor='';"" onClick=""Retorna('" & prCOD_PLANO_CONTA & "','" & prNOME & "');"">")
	Response.Write("	<td></td>")
	Response.Write("	<td style='text-align:right;'>" & prCOD_PLANO_CONTA & "</td>")
...
Sub ExibeFilhos(prCOD_PLANO_CONTA, prCOLOR)
	Dim objRSAux
	
	strSQL =          " SELECT COD_PLANO_CONTA, COD_REDUZIDO, NOME, NIVEL, DT_INATIVO "
	strSQL = strSQL & " FROM FIN_PLANO_CONTA WHERE COD_PLANO_CONTA_PAI = " & prCOD_PLANO_CONTA
	...
	while not objRSAux.Eof
		...
		ExibeLinha GetValue(objRSAux,"COD_PLANO_CONTA"), GetValue(objRSAux,"COD_REDUZIDO"), GetValue(objRSAux,"NOME"), GetValue(objRSAux,"NIVEL"), GetValue(objRSAux,"DT_INATIVO"), prCOLOR
		ExibeFilhos GetValue(objRSAux,"COD_PLANO_CONTA"), prCOLOR
...
If strCOD_RAIZ = "" Then
	strSQL = " SELECT COD_PLANO_CONTA, COD_REDUZIDO, NOME, NIVEL, DT_INATIVO FROM FIN_PLANO_CONTA WHERE COD_PLANO_CONTA_PAI IS NULL ORDER BY COD_REDUZIDO, ORDEM, NOME "
	Set objRS = objConn.Execute(strSQL)
	If Not objRS.Eof Then strCOD_RAIZ = GetValue(objRS, "COD_PLANO_CONTA")
...
<tr> 
	...
	<div class="form_line">
		<form name="form_principal" method="post" action="BuscaPlanoConta.asp">
			...
			<select name="var_cod_raiz" size="1" class="edtext_combo" style="width:180px;">
				<% montaCombo "STR", " SELECT COD_PLANO_CONTA, NOME FROM FIN_PLANO_CONTA WHERE COD_PLANO_CONTA_PAI IS NULL ORDER BY COD_REDUZIDO ", "COD_PLANO_CONTA", "NOME", strCOD_RAIZ %>
			</select>
...
<%
If strCOD_RAIZ <> "" Then
	strSQL = " SELECT COD_PLANO_CONTA, COD_REDUZIDO, NOME, NIVEL, DT_INATIVO FROM FIN_PLANO_CONTA WHERE COD_PLANO_CONTA_PAI IS NULL "
	If strCOD_RAIZ <> "" Then strSQL = strSQL & " AND COD_PLANO_CONTA = " & strCOD_RAIZ
	strSQL = strSQL & " ORDER BY COD_REDUZIDO, ORDEM, NOME "
...
<%
		...
		while not objRS.Eof
			'ExibeLinha GetValue(objRS,"COD_PLANO_CONTA"), GetValue(objRS,"COD_REDUZIDO"), GetValue(objRS,"NOME"), GetValue(objRS,"NIVEL"), GetValue(objRS,"DT_INATIVO"), strCOLOR
			ExibeFilhos GetValue(objRS,"COD_PLANO_CONTA"), strCOLOR
			objRS.MoveNext
		wend
%>
 </tbody>
