...
Response.Expires = -1
 VerificaDireito "|VIEW|", BuscaDireitosFromDB("modulo_FIN_EXTRATO", Request.Cookies("VBOSS")("ID_USUARIO")), true %>
<!--#include file="../_database/athUtils.asp"--> 
<!--#include file="_includeMontaSQLs.asp"--> 
<%
 Dim objConn, objRS, strSQL, objRSAux
 Dim strMES, strMES_NEXT, strANO, strCOD_CONTA
 ...
 if strANO <> "" and strCOD_CONTA <> "" then
	...
	strSQL = MontaSQLUnion_A(strCOD_CONTA, strDT_INI, strDT_FIM, strANO)
	'athDebug "<br><br>MontaSQLUnion_A: " & strSQL, False
	...
	strSQL = MontaSQLUnion_B(strCOD_CONTA, strDT_INI, strDT_FIM, strANO)
	'athDebug "<br><br>MontaSQLUnion_B: " & strSQL, False
	...
	'athDebug "<br><br>MontaSQLUnion_C: " & strSQL, False
	...
	'athDebug "<br><br>MontaSQLUnion_D: " & strSQL, False
	...
	Function BuscaSaldo(prCOD_CONTA, prMES, prANO)
		Dim strSQLlocal, objRSlocal1, objRSlocal2
		Dim strSALDO, strVALOR
		
		strSALDO = 0
		
		strSQLlocal = " SELECT COD_CONTA FROM FIN_CONTA WHERE COD_CONTA = " & prCOD_CONTA
		...
		if not objRSlocal1.eof then
			while not objRSlocal1.eof
				if prMES <> "" then 					
					...
					strSQLlocal = strSQLlocal & " FROM FIN_SALDO_AC WHERE STR_TO_DATE(CONCAT('01 ', MES, ' ', ANO),'%d %m %Y') < STR_TO_DATE(CONCAT('01 ', " & prMES & ", ' ', " & prANO & "),'%d %m %Y') "
					strSQLlocal = strSQLlocal & " AND COD_CONTA=" & GetValue(objRSlocal1,"COD_CONTA") 
...
<%	
		...
		auxDiaANT = ""
  		while i < intTamNew
			If i = 0 Then
				strSALDO_ANTERIOR = BuscaSaldo(strCOD_CONTA, strMES, strANO)
 ...
 ' INI: Mostra SALDO do dia (como num extrato bancario - ITAÚ) --------
  if (matRS(DATA,i) <> auxDiaANT) then 
%>
	<tr>
		<td colspan="6"></td>
		<td align="right">saldo</td>
		<td align="right" bgcolor="#C1DAD7" colspan="2"><%=FormataDecimal(strSALDO_ANTERIOR-(-strSALDO_PARCIAL),2)%></td>
		<td></td>
	</tr>	
<% 
  End IF 
  auxDiaANT = matRS(DATA,i)
 ' FIM: Mostra SALDO do dia ------------------------------------------
			...
			strPLANOCONTA = matRS(NOME_PLANO_CONTA ,i)
			If matRS(COD_PLANO_CONTA ,i) <> "" Then strPLANOCONTA = strPLANOCONTA & "&nbsp;(" & matRS(COD_PLANO_CONTA ,i) & ")"
...
%>
	...
	<tr bgcolor="<%=strCOLOR%>">
		<td>
		<%
		If matRS(MODULO,i) <> "LCTOTRANSF" Then
			Response.Write(MontaLinkGrade("modulo_FIN_"& matRS(MODULO,i),"Detail.asp",matRS(CODIGO,i),"IconAction_DETAIL.gif","DETALHE COM INSERÇÃO DE LCTO"))
		Else
			Response.Write(MontaLinkGrade("modulo_FIN_LCTOCONTA","DetailTransf.asp",matRS(CODIGO,i),"IconAction_DETAIL.gif","DETALHE COM INSERÇÃO DE LCTO"))
		End If
		%>
		</td>
