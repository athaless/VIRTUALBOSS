...
<tr> 
 	<td width="1%" class="top_menu" style="background-image:url(../img/Menu_TopBgLeft.jpg); vertical-align:top; padding:10px 0px 0px 10px;  border-collapse:collapse;">
	    <b><!-- <a href="Help.htm" target="vbMainFrame" title="sobre este módulo...">[?]&nbsp;</a>-->Backlog</b><br>
	...
	<td width="98%" class="top_filtros" style="background-image:url(../img/Menu_TopBgRight.jpg); vertical-align:bottom; padding:0px 5px 5px 0px; margin:0px; text-align:right; border:none; border-collapse:collapse;">
		<div class="form_line">
			<form name="form_principal" id="form_principal" method="get" action="<%=CFG_MAIN_GRID%>" target="vbMainFrame">
				<!--
				<div class="form_label_nowidth">Cód Projeto:</div><input name="var_cod_projeto" type="text" size="20" class="edtext">
                //-->
				<select name="var_cod_projeto" class="edtext_combo" style="width:80px;">
					<option value="">[projeto]</option>
					<%=montaCombo("STR","SELECT cod_projeto,titulo FROM prj_projeto", "COD_PROJETO", "TITULO", "")%>
				</select>
				<div class="form_label_nowidth">Título:</div><input name="var_titulo" type="text" size="20" class="edtext">
				<div class="form_label_nowidth">Descrição:</div><input name="var_descricao" type="text" size="20" class="edtext">				
				<select name="var_status" class="edtext_combo" style="width:80px;">
					<option value="">[status]</option>
					<%=montaCombo("STR","SELECT DISTINCT (status) FROM prj_backlog", "STATUS", "STATUS", "")%>
				</select>
				<!-- Para diminuir ou eliminar a ocorrência de cache passamos um parâmetro DUMMY com um número diferente 
