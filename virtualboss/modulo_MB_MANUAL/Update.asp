...
<% VerificaDireito "|UPD|", BuscaDireitosFromDB("modulo_MB_MANUAL", Request.Cookies("VBOSS")("ID_USUARIO")), true %>
...
<%
	...
	if strCODIGO <> "" then 
		strSQL =          " SELECT COD_MANUAL, TITULO, AUTORES, ID, CDU, CDD, EDITORA, EDICAO, ANO, AQUISICAO, CODBAR, ISBN, IMG, LOCALIZACAO, PRAZO_EMPR, IDIOMA, COD_CATEGORIA, ASSUNTO, CLASSE, VOLUME, PROPRIEDADE, EXTRA, RESENHA, DT_INATIVO, LOCADO "
		strSQL = strSQL & " FROM mb_manual "
		strSQL = strSQL & " WHERE COD_MANUAL = " & strCODIGO
...
<body>
<%=athBeginDialog(WMD_WIDTH, "Manual - Altera&ccedil;&atilde;o") %>
<form name="form_update" action="../_database/athUpdateToDB.asp" method="post">
	...
	<input type="hidden" name="DEFAULT_LOCATION" value="../modulo_MB_MANUAL/Update.asp?var_chavereg=<%=strCODIGO%>">
	...
	<div class="form_grupo" id="form_grupo_1">
		...
		<br><div class="form_label">*Edição:</div><select name="DBVAR_STR_EDICAOô" style="width:50px;"><option value="" <% if (CStr(GetValue(objRS,"EDICAO"))="")then Response.Write("selected=""selected""") end if %> ></option>
		...
		<br><div class="form_label">Categoria:</div><select name="DBVAR_INT_COD_CATEGORIA" style="width:140px;"><option value="NULL"></option><%=montaCombo("STR","SELECT COD_CATEGORIA, NOME FROM mb_manual_categoria WHERE DT_INATIVO IS NULL","COD_CATEGORIA","NOME",GetValue(objRS,"COD_CATEGORIA"))%></select>
		...
		<br><div class="form_label">Imagem:</div><input name="DBVAR_STR_IMG" type="text" maxlength="250" value="<%=GetValue(objRS,"IMG")%>" style="width:200px;"><a href="javascript:UploadArquivo('form_update','DBVAR_STR_IMG', 'upload//<%=Request.Cookies("VBOSS")("CLINAME")%>//MB_MANUAL//');"><img src="../img/BtUpload.gif" border="0" style="vertical-align:top; padding-top:2px;" vspace="0" hspace="0"></a>									 
	</div>
