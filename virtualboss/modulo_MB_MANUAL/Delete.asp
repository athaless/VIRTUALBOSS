...
<% VerificaDireito "|DEL|", BuscaDireitosFromDB("modulo_MB_MANUAL", Request.Cookies("VBOSS")("ID_USUARIO")), true %>
...
' -------------------------------------------------------------------------------
   ...
   If strCODIGO <> "" Then
	  ...
	  strSQL = " SELECT * FROM mb_manual WHERE COD_MANUAL = " & strCODIGO
...
<body>
	<%=athBeginDialog(WMD_WIDTH, "Manual - Dele&ccedil;&atilde;o")%>
<% for Idx = 0 to objRS.fields.count - 7 'NÃO TRAZER TODOS OS DADOS %> 
	...
	<div class="form_bypass">
		<% if(objRS.Fields(Idx).name = "IMG")then %>
			<img src='../upload/<%=Request.Cookies("VBOSS")("CLINAME")%>/MB_MANUAL/<%=GetValue(objRS,"IMG")%>' width='160' />
		<% else %>
