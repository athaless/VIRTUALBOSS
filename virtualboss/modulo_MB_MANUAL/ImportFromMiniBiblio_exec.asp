...
<%
 ...
 'COD|TITULO|AUTOR|EDITORA|EDICAO|ANO|CODBAR|ISBN|LOCAL|PRAZO EMPR|LOCADO|IDIOMA|AQUISICAO|CATEGORIA|ASSUNTO|CLASSE|VOLUME|EXTRA|RESENHA
 If ucase(strLine)<>"XLW|GRGFOL|ZFGLI|VWRGLIZ|VWRXZL|ZML|XLWYZI|RHYM|OLXZO|KIZAL VNKI|OLXZWL|RWRLNZ|ZJFRHRXZL|XZGVTLIRZ|ZHHFMGL|XOZHHV|ELOFNV|VCGIZ|IVHVMSZ" then
 ...
 function GetCodCateg(prStrCateg)
	  strSQL = "SELECT COD_CATEGORIA FROM mb_manual_categoria WHERE NOME like '" & prStrCateg & "'"
	  set objRS = objConn.Execute(strSQL)  
	  If Not objRS.EOF Then
		  GetCodCateg = GetValue(objRS,"COD_CATEGORIA")
	  Else  
		  strSQL = "INSERT INTO mb_manual_categoria (NOME) VALUES ('" & prStrCateg & "')"
		  set objRS = objConn.Execute(strSQL)  
 ...
 do while objARQ.AtEndOfStream = false
    ...
    strSQL = strSQL & "'" &	RetValue4SQl(arrLine(AUTOR),"")					& "',"
