	...
	<script type="text/javascript" language="JavaScript">
		...
		var form = eval('document.' + pr_form + '.' + pr_input);
			if (form.value=='INSERIR') { 
			 ...
			 parent.frames["vbMainFrame"].document.location.href = "ImportFromMiniBiblio.asp"; 
			}
			form.value='';
		}
	</script>
...
<tr> 
	<td width="1%" class="top_menu" style="background-image:url(../img/Menu_TopBgLeft.jpg); vertical-align:top; padding:10px 0px 0px 10px;  border-collapse:collapse;">
	    <b>Manuais</b>
		<%=montaMenuCombo("form_acoes","selNome","width:100px","ExecAcao(this.form.name,this.name);","INSERIR:INSERIR;IMPORTAR:IMPORTAR")%>
	</td>
	...
	<td width="98%" class="top_filtros" style="background-image:url(../img/Menu_TopBgRight.jpg); vertical-align:bottom; padding:0px 5px 5px 0px; margin:0px; text-align:right; border:none; border-collapse:collapse;">
		<div class="form_line">
			<form name="form_principal" id="form_principal" method="get" action="<%=CFG_MAIN_GRID%>" target="vbMainFrame">
                <div class="form_label_nowidth">Cod.:</div><input name="var_codigo" type="text" size="5" class="edtext">
				<div class="form_label_nowidth">Titulo:</div><input name="var_titulo" type="text" size="20" class="edtext">
				<div class="form_label_nowidth">Autores:</div><input name="var_autores" type="text" size="15" class="edtext">
				<div class="form_label_nowidth">Extra:</div><input name="var_extra" type="text" size="10" class="edtext">
