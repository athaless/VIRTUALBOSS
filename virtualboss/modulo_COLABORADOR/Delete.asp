...
<% VerificaDireito "|DEL|", BuscaDireitosFromDB("modulo_COLABORADOR", Request.Cookies("VBOSS")("ID_USUARIO")), true %>
...
<%
 ...
 Const auxAVISO  = "dlg_warning.gif:ATENÇÃO!Você está prestes a remover o registro <br> acima visualizado. Para confirmar clique no botão [ok], para desistir clique em [cancelar]."
  ...
  If strCODIGO <> "" Then
	  ...
	  strSQL = "SELECT * FROM ENT_COLABORADOR WHERE COD_COLABORADOR = " & strCODIGO 
...
<body>
<%=athBeginDialog(WMD_WIDTH, "Colaborador - Dele&ccedil;&atilde;o")%>     
<% for Idx = 0 to objRS.fields.count - 31 'NÃO QUIZ EXIBIR TODOS OS DADOS... %>
