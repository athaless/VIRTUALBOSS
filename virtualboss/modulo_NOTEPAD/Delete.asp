...
<% 
 ...
 'VerificaDireito "|DEL|", BuscaDireitosFromDB("modulo_NOTEPAD", Request.Cookies("VBOSS")("ID_USUARIO")), true %>
...
<%
 ...
 Const auxAVISO  = "dlg_warning.gif:ATENÇÃO!Você está prestes a remover o registro <br> acima visualizado. Para confirmar clique no botão [ok], para desistir clique em [cancelar]."
   ...
   If strCODIGO <> "" Then
	  ...
	  strSQL = "SELECT * FROM NOTEPAD WHERE COD_NOTEPAD = " & strCODIGO
...
<body>
<%=athBeginDialog(WMD_WIDTH, "Anotações - Dele&ccedil;&atilde;o")%>     
<% for Idx = 0 to objRS.fields.count - 1  'NÃO QUIZ EXIBIR TODOS OS DADOS... %> 
<br><div class="form_label"><%=objRS.Fields(Idx).name%>:</div><span class="form_bypass"><%=GetValue (objRS, objRS.Fields(Idx).name)%></span>
<% next %>
